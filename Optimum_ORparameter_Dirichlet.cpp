...
void set_outer_boundary(std::vector<double>& phi, std::vector<int>& phi_fixed, int nx, int ny, double hx, double hy) 
{
    for (int i = 0; i < nx; ++i){
        phi[idx(i, 0, nx)] = 0.0;
        phi_fixed[idx(i, 0, nx)] = 1;           
    }

    for (int j = 0; j < ny; ++j){
        phi[idx(0, j, nx)] = 0.0;
        phi_fixed[idx(0, j, nx)] = 1;
    }
...
void set_inner_boundary(std::vector<double>& phi, std::vector<int>& phi_fixed, int nx, int ny, double hx, double hy) 
{
    ...
    for (int it = 1; it <= max_iterations; ++it) {
        ...
        for (int j = 1; j < ny - 1; ++j) {
            for (int i = 1; i < nx - 1; ++i) {
                int id = idx(i, j, nx);
                if (phi_fixed[id] != 0) continue; // skip the dirichlet points in the region
...
int main() {
    ...
    std::ofstream csv("omega_sweep.csv");
    csv << "w,iterations,final_change\n";
    ...
    for (int r = 0; r < wsteps; ++r) {
        ...
        sor_method(phi, phi_fixed, nx, ny, w, max_iterations, it_used, final_change);

         
        std::cout << " w = " << w << " Iterations = " << it_used << " final change = " << final_change << std::endl;

        csv << w << "," << it_used << "," << final_change << "\n";
